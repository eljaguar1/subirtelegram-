...
#Estan importadas arriba
#from aiogram import Bot, Dispatcher, executor, types 
#from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
# Put the token that you received from BotFather in the quotes 
bot = Bot(token='7310109099:AAFmFJ-MqduqMsDomgGCNBsLZx_uTrB55Z0')

# Initializing the dispatcher object 
dp = (Dispatcher)(bot=bot)

# Defining and adding buttons 
button1 = InlineKeyboardButton(text="🔛Iniciar", callback_data="start")
button2 = InlineKeyboardButton(text="🏓Ping", callback_data="ping")
button3 = InlineKeyboardButton(text="🆘Ayuda", callback_data="help") 
button4 = InlineKeyboardButton(text="🗂Lista", callback_data="ls")
button5 = InlineKeyboardButton(text="🗂Atrás", callback_data="back")
button6 = InlineKeyboardButton(text="🚀Auto", callback_data="auto")
button7 = InlineKeyboardButton(text="📊Estado", callback_data="status")
button8 = InlineKeyboardButton(text="☁️Modo", callback_data="modo")
button9 = InlineKeyboardButton(text="❌No proxy", callback_data="offproxy")
button9 = InlineKeyboardButton(text="🎛️Comandos", callback_data="command")
keyboard_inline = InlineKeyboardMarkup().add(button1, button2, button3, button4, button5, button7) 
...
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)
        ...
        # Message handler for the /button2 command
@dp.message_handler(commands=['ping'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

    # Message handler for the /button3 command
@dp.message_handler(commands=['help'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

    # Message handler for the /button4 command
@dp.message_handler(commands=['ls'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

    # Message handler for the /button5 command
@dp.message_handler(commands=['back'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

    # Message handler for the /button6 command
@dp.message_handler(commands=['auto'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

    # Message handler for the /button7 command
@dp.message_handler(commands=['status'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

    # Message handler for the /button8 command
@dp.message_handler(commands=['modo'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

    # Message handler for the /button9 command
@dp.message_handler(commands=['offproxy'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

    # Message handler for the /button10 command
@dp.message_handler(commands=['command'])
async def check(message: types.Message):
	await message.reply("", reply_markup=keyboard_inline)

# Callback query handler for the inline keyboard buttons 
@dp.callback_query_handler(text=["In_2_button"])
async def check_button(call: types.CallbackQuery):

	# Checking which button is pressed and respond accordingly
	if call.data == "In_2_button":
